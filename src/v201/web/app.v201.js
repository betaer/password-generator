...
立即体验：https://betaer.github.io/password-generator/v2.01.html`;
  ...
  const SCHEME_LABELS = Object.freeze({
    password: '均匀约束密码模型 V2.0.1', passphrase: '独立词池口令模型 V2.0.1', pin: '均匀约束 PIN 码模型 V2.0.1',
    token: '随机字节令牌模型 V2.0.1', 'api-secret': 'API 密钥模型 V2.0.1', hex: '十六进制随机字节模型 V2.0.1',
    'random-bytes': '原始随机字节模型 V2.0.1', uuid: 'RFC 9562 UUID 标识符模型', mnemonic: 'BIP39 助记词模型 V2.0.1',
  });
  ...
  const state = {
    ...
    resources: {
      ...
      passphrase: { status: 'loading', detail: 'V2.0.1 口令词包' },
  ...
  const CONFIG_TEMPLATES = Object.freeze({
    password: () => `<div class="field-grid">
      <div class="field"><label for="length">密码长度</label><input id="length" name="length" type="number" min="1" max="4096" value="20"><small>整批只编译一次概率模型并在一个工作线程中采样。</small></div>${quantityMarkup()}
  ...
  async function initializePassphrase() {
    setResource('passphrase', 'loading', 'V2.0.1 口令词包');
    try {
      ...
      setResource('passphrase', 'ready', 'V2.0.1 口令词包');
      if (state.mode === 'passphrase') renderConfig();
    } catch (error) { setResource('passphrase', 'error', 'V2.0.1 口令词包'); showToast(error.message, 'error'); }
  }
  ...
  function localizedModelDetails(result, assessment) {
    return {
      ...
      方案标识: SCHEME_LABELS[result.type] || 'V2.0.1 专用安全生成模型',
