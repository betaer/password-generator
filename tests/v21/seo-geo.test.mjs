...
test('V2.1 SEO 元信息完整度与 V1.7.5 对齐且规范地址唯一', async () => {
  ...
  for (const pattern of [
    ...
    new RegExp(`<link rel="alternate" hreflang="en" href="${canonical.replaceAll('.', '\\.')}`),
...
test('V2.1 提供完整 Open Graph、Twitter 和九类生成器 JSON-LD', async () => {
  ...
  for (const pattern of [
    ...
    /<meta property="og:locale:alternate" content="en_US"/u,
    /<meta property="og:title"/u,
    /<meta property="og:description"/u,
    new RegExp(`<meta property="og:url" content="${canonical.replaceAll('.', '\\.')}`),
    /<meta property="og:image" content="https:\/\/betaer\.github\.io\/password-generator\/assets\/social-preview\.png"/u,
  ...
  await access(new URL('assets/social-preview.png', root));
});
