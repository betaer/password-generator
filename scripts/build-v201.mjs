...
const outputDirectory = path.join(projectRoot, 'assets/v2.01');
const sourceDirectory = path.join(projectRoot, 'src/v201/web');
const VERSION = '2.0.1';
...
async function buildPinRiskAsset() {
  ...
  const source = `/* V2.0.1 PIN heuristic policy asset. */\n(function(root){'use strict';var assets=root.PasswordGeneratorV201Assets||(root.PasswordGeneratorV201Assets={});assets.pinRisk=Object.freeze(${javascriptJson(built)});})(globalThis);\n`;
  return writeHashed('pin-risk', 'js', source);
}

async function buildAnalyticsFrame() {
  let html = await readFile(path.join(sourceDirectory, 'analytics-frame.v201.html'), 'utf8');
  const match = html.match(/<script id="v201-analytics-config">([\s\S]*?)<\/script>/u);
  if (!match) throw new Error('Missing V2.0.1 analytics config script');
  const cspHash = `sha256-${sha256(match[1], 'base64')}`;
  html = html.replace('__V201_ANALYTICS_HASH__', cspHash);
  return writeHashed('analytics-frame', 'html', html);
}

async function buildPasswordWorker(runtimeFile) {
  const source = (await readFile(path.join(sourceDirectory, 'password-worker.v201.js'), 'utf8'))
    .replace('__V201_RUNTIME_FILE__', javascriptJson(runtimeFile));
  return writeHashed('password-worker', 'js', source);
}

async function buildPage(assets) {
  const replacements = {
    __V201_CSS__: assets.css,
    __V201_PASSWORD_WORKER__: assets.passwordWorker,
    __V201_ZXCVBN_WORKER__: assets.zxcvbnWorker,
    __V201_PASSPHRASE__: assets.passphrase,
    __V201_PIN_RISK__: assets.pinRisk,
    __V201_BIP39_ENGLISH__: assets.bip39.english,
    __V201_ANALYTICS_FRAME__: assets.analyticsFrame,
    __V201_RUNTIME__: assets.runtime,
    __V201_APP__: assets.app,
  };
  let html = await readFile(path.join(sourceDirectory, 'page.v201.html'), 'utf8');
  for (const [placeholder, value] of Object.entries(replacements)) html = html.replaceAll(placeholder, value);
  if (/__V201_[A-Z_]+__/u.test(html)) throw new Error('Unresolved V2.0.1 page placeholder');
  await writeFile(path.join(projectRoot, 'v2.01.html'), html);
}

export async function buildV201() {
  ...
  const [zxcvbnWorker, bip39, passphrase, pinRisk, analyticsFrame, css] = await Promise.all([
    ...
    readFile(path.join(sourceDirectory, 'app.v201.css')).then((bytes) => writeHashed('app', 'css', bytes)),
  ]);
  ...
  const appBytes = await bundle(path.join(sourceDirectory, 'app.v201.js'), { define: { __V201_ASSET_MANIFEST__: JSON.stringify(appManifest) } });
...
if (isMain) {
  const assets = await buildV201();
  process.stdout.write(`Built V2.0.1: ${Object.keys(assets).length} asset groups\n`);
}
